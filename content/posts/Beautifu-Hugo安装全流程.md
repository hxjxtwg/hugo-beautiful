...
body.dark-mode .highlight {
    ...
    border: 1px solid #333 !important;        /* 深色模式下稍微有点边框好看些，不要可删 */
}
...
```
或者把/* 6. 代码块逻辑 ... */ 到 /* 7. Halo 赛车动画 ... */ 之间的内容替换为：
```html
/* ======================================================= */
/* 6. 代码块逻辑 (修复版：去双层背景，统一单色) */
/* ======================================================= */
...
/* 2. 浅色模式 (Light Mode) 统一背景 */
/* 给最外层容器设置统一的浅灰背景 */
.highlight {
    background-color: #f5f5f5 !important; /* 这里修改你想要的背景色 */
    border-radius: 6px !important;
    border: none !important;
}

/* ★★★ 核心修复：强制内部所有元素背景透明 ★★★ */
/* 这样就能透出最外层的 #f5f5f5，消除“双眼皮”色差 */
.highlight div, 
.highlight pre, 
.highlight code, 
.highlight table, 
...
```html
/* ====================================================== */
    /* 代码块滚动条优化 (Beautiful Hugo 移动端修复版) */
    /* ====================================================== */

    /* --- 1. PC端逻辑 (保持你满意的效果：平时隐藏，悬停显示) --- */
    @media (min-width: 769px) {
        .highlight pre::-webkit-scrollbar,
        pre::-webkit-scrollbar {
            height: 6px;       /* PC端稍微粗一点点，方便鼠标拖动 */
            width: 6px;
    ...
    /* --- 2. 移动端逻辑 (强制彻底隐藏，保持触摸滑动) --- */
    @media (max-width: 768px) {
        /* Webkit内核 (Chrome/Safari/安卓) 强制不显示滚动条元素 */
        .highlight pre::-webkit-scrollbar,
        pre::-webkit-scrollbar {
            display: none !important; /* 核心代码：直接不渲染滚动条 */
            width: 0 !important;
            height: 0 !important;
            background: transparent !important;
        }

        /* 容器本身设置 (兼容 Firefox 和部分安卓) */
        .highlight pre, pre {
            /* 允许触摸滑动 */
            overflow-x: auto;
...
```
此方法是在登陆github帐号的情况下切换仓库
如果推送出现网络故障可以开启代理,以v2rayN的端口为例。
```bash
